import { arrayDifference, ContainerRegistrationKeys, MedusaError, MedusaErrorTypes, Modules } from "@medusajs/framework/utils";
...
createProductsWorkflow.hooks.productsCreated(
    async ({ products, additional_data }, { container }) => {
        ...
        const existentAttributeValues = await attributeModuleService.listAttributeValues({
            id: attributeValueIds,
        })

        const inexistentAttributeValues = arrayDifference(attributeValueIds, existentAttributeValues.map(val => val.id))
        ...
        const { data: attributeCategory } = await query.graph({
            ...
            filters: {
                attribute_id: existentAttributeValues.map(attrVal => attrVal.attribute_id)
            }
        })
        ...
        for (const attributeValue of existentAttributeValues) {
            const attributeCategories = attributeCategoriesMap.get(attributeValue.attribute_id)
            if (!attributeCategories) {
                continue
            }

            if (productCategoryIds.some(prodCat => attributeCategories.includes(prodCat))) {
                continue
            }
